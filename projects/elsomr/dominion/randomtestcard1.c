...
int checkBaronCard(int choice1, int Numplayers, int p, struct gameState *post) {
	...
	if (choice1 > 0)	{
		printf("Coin check = %s\n", assert(pre.coins, post.coins, failCounter));
		printf("Hand Increases Check = %s\n", assert(pre.handCount[p]+1, post.handCount[p], failCounter));
		printf("Card is put in discard pile check = %s\n", assert(estate, post.disard[p][post.discardCount[p]], failCount));
		printf("Estate supply decreases = %s\n", assert(pre.supplyCount[estate], post.supplyCount[estate], failCounter));
		...
		printf("Hand is static = %s\n", assert(pre.handCount[p], post.handCount[p], failCounter));
