...
int baronEffect(int choice1, struct gameState* state)	{
		...
		while(card_not_discarded) {
			if (state->hand[currentPlayer][p] == estate) { //Found an estate card!
				...
				if (supplyCount(estate, state) > 0) {
					...
					/*===================================================
							Bug for Assignment 2 (one line below, uncomment)
					=====================================================*/
	...
	//if user chose to gain an estate instead of selling
	else {
		if (supplyCount(estate, state) > 0) {
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    {
	case baron:
		//creating a function that replicates the baron card effect
	...
	case minion:
		minionEffect(state, handPos, choice1, choice2);
		return 0;
    ...
    case ambassador:
		ambassadorEffect(choice2, choice1, state, handPos);
        return 0;
